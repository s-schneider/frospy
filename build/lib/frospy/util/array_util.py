...
from frospy.util.base import nextpow2, stream2array, array2stream, array2trace
...
def alignon(st, inv=None, event=None, phase=None, ref=0, mtw=0,
            xcorr=False, shiftmethod='normal', taup_model='ak135',
            verbose=False):
    """
    Aligns traces on a given phase and truncates the starts to the latest
    beginning and the ends to the earliest end.
    ...
    :param ref: name or index of reference station, to which the others are
                aligned
    :type ref: int or str

    :param mtw: Maximum timewindow in seconds, symmetrical around
                          theoretical phase arrival time, in which to pick the
                          maximum amplitude.  Also possible to input a list
                          with time before and after the theoretical pick.

    :type mtw: int, float or string

    :param xcorr: Use cross correlation with the reference trace to align
                  traces.
    :type  xcorr: bool

    :param taup_model: model used by TauPyModel to calculate arrivals,
                       default is ak135
    :type taup_model: str
    ...
    if not isinstance(event, Event):
        if isinstance(phase[0], int) and isinstance(phase[1], int):
            timewindow = True
    ...
    if isevent:
        ...
        if isinstance(mtw, list):
            mtw = np.array(mtw)
        elif isinstance(mtw, int):
            mtw = float(mtw)

        # Calculating reference arriving time/index of phase.
        ttime = m.get_travel_times(depth, ref_dist, phase_list=phase)[0].time
        ref_t = origin + ttime - ref_start
        ref_n = int(ref_t / delta)

        if xcorr:
            if isinstance(mtw, np.ndarray):
                reftrace_tmp = cut(st_tmp[iref], ref_t - abs(mtw[0]),
                                   ref_t + mtw[1])
                reftrace = reftrace_tmp.data

            elif isinstance(mtw, float):
                reftrace_tmp = cut(st_tmp[iref], ref_t - mtw,
                                   ref_t + mtw)
                reftrace = reftrace_tmp.data

            datashift_null, shift_index = shift2ref(data[iref, :], ref_n,
                                                    ref_n, ref_array=reftrace,
                                                    mtw=mtw / delta,
                                                    method=shiftmethod,
                                                    xcorr=xcorr)
        ...
        # First work on reference Trace:
            ...
            tmtw = type(mtw)
            if tmtw == float or tmtw == int or tmtw == np.ndarray:
                datashift_null, shift_index = shift2ref(data[iref, :], ref_n,
                                                        ref_n,
                                                        mtw=mtw/delta,
                                                        method=shiftmethod)
        ...
        for no_x, data_x in enumerate(data):
            ...
            if not xcorr:
                tmtw = type(mtw)
                if tmtw == float or tmtw == int or tmtw == np.ndarray:
                    datashift, shift_index = shift2ref(data_x, ref_n, phase_n,
                                                       mtw=mtw / delta,
                                                       method=shiftmethod)
            ...
            if verbose:
                msg = 'Trace no %i was shifted by %f seconds'
                print(msg % (no_x, delta * shift_index))
            # Positive shift_index indicates positive shift in time
            # and vice versa.
    ...
    # Alignment of timewindow around
        ...
        mtw = np.array([0, phase[1] - phase[0]])

        if xcorr:
            if isinstance(mtw, np.ndarray):
                rt_tmp = cut(st_tmp[iref], ref_t-abs(mtw[0]), ref_t+mtw[1])
                reftrace = rt_tmp.data

            elif isinstance(mtw, float):
                rt_tmp = cut(st_tmp[iref], ref_t - mtw, ref_t + mtw)
                reftrace = rt_tmp.data
        ...
        msg = 'No valid input defined, please use event-file or'
        msg += ' time-window defined in phases'
        print(msg)
        return
...
def aperture(inventory):
    ...
    Method:find the maximum of the calculation of distance of every possible
...
def attach_coordinates_to_traces(stream, inventory, event=None):
    ...
    if event:
        ...
        msg = '"No Event information found, distance, origin and '
        msg += 'No Event information found, distance, origin and back-azmuth '
        msg += 'will NOT be set!""back-azmuth will NOT be set!"'
        print(msg)
...
def attach_epidist2coords(inventory, event, stream=None):
    """
    Receives the epicentral distance of the station-source couple given in
    inventory - event and adds them to Array_Coords.
    If called with stream, it uses just the coordinates of the used stations
    in stream.
    ...
    try:
        ...
        for network in inv:
            for station in network:
                ...
                Array_Coords[scode]["epidist"] = locations2degrees(lat1, lat2,
                                                                   eventlat,
                                                                   eventlon)
            ...
            for network in inventory:
                for station in network:
                    if station.code != trace.meta.station:
                        continue
                    trace.stats.network = network.code

                    if station.channels:
                        for chan in station.channels:
                            if trace.stats.channel == chan.code:
                                tst = trace.stats.starttime
                                if tst > chan.start_date:
                                    if tst < chan.end_date:
        ...
        for network in inventory:
            for station in network:
                if station.code != stream.meta.station:
                    continue
                stream.stats.network = network.code

                if station.channels:
                    for chan in station.channels:
                        if stream.stats.channel == chan.code:
                            tst = stream.stats.starttime
                            if tst > chan.start_date:
                                if tst < chan.end_date:
                                    loc = chan.location_code
...
def epidist2nparray(Array_Coords):
    ...
    for scode in Array_Coords:
        if Array_Coords[scode]["epidist"]:
            epidist_np = np.append(epidist_np,
                                   [Array_Coords[scode]["epidist"]])
...
def find_closest_station(inventory, stream, latitude, longitude,
    ...
    Calculates closest station of an inventory to a given latitude, longitude
    and absolute_height_in_km
    ...
    for network in inventory:
        for i, station in enumerate(network):
            distance = np.sqrt(((gps2dist_azimuth(lats[i], lngs[i], x, y)[0]) /
                               1000.0) ** 2 + (np.abs(np.abs(z) -
                                               np.abs(hgt[i]))) ** 2)
...
def gaps_fill_zeros(stream, inv, event, decimal_res=1):
    """
    WARNING: Use this method only for synthetics, for real data prefer
    bowpy.util.fkutil.partial_stack
    ...
    try:
        ...
        try:
            ...
            msg = "Need inventory and event or distance information in "
...
def get_coords(inventory, returntype="dict"):
    """
    Get the coordinates of the stations in the inventory, independently of the
    channels, better use for arrays, than the channel-dependent
    core.inventory.inventory.Inventory.get_coordinates().

    returns the variable coords with entries:
        elevation (in km), latitude and longitude.
    :param inventory: Inventory to get the coordinates from
    :type inventory: obspy.core.inventory.inventory.Inventory

    :param coords: dictionary with stations of the inventory and its
                   elevation (in km), latitude and longitude
    :type coords: dict
...
def isuniform(inv, event, stream=None, tolerance=0.5):
    """
    Checks if the epicentral station distribution is uniform, in a given
    tolerance range.
    ...
    :param stream: stream-data, optional. If stream is an input only stations
                   in stream will be used for calculation
...
def resample_distance(stream, inv=None, event=None, shiftmethod='fft',
                      taup_model='ak135', stacking=False, refphase='PP'):
    """
    ...
    yres = np.linspace(ymin, ymax, npts)
    ilist = []
    tstart_new_list = []
    ...
    for no, trace in enumerate(st_tmp):
        ...
        if stacking:
            index_resampled = np.abs(yres - trace.stats.distance).argmin()
        else:
            index_resampled = no
        if refphase:
            torg = m.get_travel_times(depth, trace.stats.distance,
                                      phase_list=[refphase])[0].time
            tres = m.get_travel_times(depth, yres[index_resampled],
                                      phase_list=[refphase])[0].time
            tdelta = torg - tres
            shiftvalue = int(tdelta / trace.stats.delta)
            trace.data, shift_index = shift2ref(data[no], 0, shiftvalue,
                                                method=shiftmethod)
        ...
        if stacking:
            if index_resampled in ilist:
                ...
                for i, stacktraces in enumerate(stream_resample):
                    if stacktraces.stats.distance == yres[index_resampled]:
        ...
        # if np.abs(yres - trace.stats.distance).min() <= ymeandiff:

        trace.stats.distance = yres[index_resampled]
        try:
    ...
    zero_traces = list(set(range(yres.size)) - set(ilist))
    if zero_traces:
        for j in zero_traces:
            ...
            newtrace.stats.distance = yres[j]
            newtrace.stats.zerotrace = "True"
...
def resample_partial_stack(st, bin_size=None, refphase='P', overlap=None,
                           ...
                           shiftmethod='normal', taup_model='ak135'):
    """
    Will sort the traces into equally distributed bins and stack the bins.
    The stacking is just an addition of the traces, more advanced schemes might
    follow. The uniform distribution is useful for FK-filtering, SSA and every
    method that requires a uniform distribution.

    Needs depth information attached to the stream, array_util.
    See attach_coordinates_to_stream() and attach_network_to_traces().

    input:
    :param st: obspy stream object
    :type st: obspy.core.stream.Stream

    :param bins: number of bins, that should be used, if overlap is set,
                it is used to calculate the size of each bin.
    :type bins: int

    :param refphase: If True, traces will be shifted to reference time of the
                     bin, calculated by the reference phase.
                     If Traces already aligned on a phase switch to False.
    :type refphase: str

    :param overlap: degree of overlap of each bin, e.g 0.5 corresponds to
                    50 percent overlap ov each bin, to deactivate set to "None"
    :type: float

    :param order: Order of Nth-root stacking, default None
    :type order: float or int

    :param maxtimewindow: Maximum timewindow in seconds, symmetrical around
                          theoretical phase arrival time,
                          in which to pick the maximum amplitude.

    :param taup_model:

    returns:
    :param bin_data: partial stacked data of the array in bins uniform
                     distributed stacks
    :type bin_data: array

    Author: S. Schneider, 2016
    Reference: Rost, S. & Thomas, C. (2002). Array seismology: Methods and
    Applications
    """
    ...
    if overlap and not isinstance(overlap, bool):
        ...
        y_resample = np.linspace(min(min(L)) + bin_size / 2., max(max(L)) -
                                 bin_size / 2., no_of_bins)
    ...
    if refphase:
        try:
            for i, res_distance in enumerate(y_resample):
                yr_sampleindex[i] = int(m.get_travel_times(depth, res_distance,
                                        phase_list=[refphase])[0].time / delta)

            for i, epi_distance in enumerate(epidist):
                yi_sampleindex[i] = int(m.get_travel_times(depth, epi_distance,
                                        phase_list=[refphase])[0].time / delta)
        except:
            for i, res_distance in enumerate(y_resample):
                yr_sampleindex[i] = int(m.get_travel_times(depth, res_distance,
                                        phase_list=[refphase+'diff'])[0].time /
                                        delta)
    ...
    for i, bins in enumerate(L):
        ...
        for j, trace in enumerate(data):
            ...
            if i == 0:
                if epidist[j] <= bins[1]:
                    if refphase:
                        trace_shift, shif_index = shift2ref(trace,
                                                            yr_sampleindex[i],
                                                            yi_sampleindex[j],
                                                            mtw,
                                                            method=shiftmethod)
...
def shift2ref(array, tref, tshift, ref_array=None, mtw=0, method='normal',
              xcorr=False):
    """
    Shifts the trace in array to the order of tref - tshift. If mtw is given,
    tshift will be calculated depdending on the maximum amplitude of array in
    ...
    Author: S. Schneider, 2016
    Source: Gubbins, D., 2004 Time series analysis and inverse theory for
    geophysicists
    """
    trace = array.copy()
    ...
    if isinstance(mtw, float) and not xcorr:
        ...
        if isinstance(mtw, float):
            ...
            tw = truncate(trace, tshift - mtw[0], tshift + mtw[1],
                          absolute=True)

        shift_value = tref - tshift - (correlate(ref_array, tw).argmax() + 1 -
                                       tw.size)
...
def truncate(data, tmin, tmax, absolute=False):
    """
    Truncates the data array on the left to tmin,
                             on the right to right-end - tmax.
...
def vespagram(stream, slomin=-5, slomax=5, slostep=0.1, inv=None, event=None,
              power=4, cmap='seismic',
              markphases=None, method='fft',
              tw=None, zoom=1, savefig=False, dpi=400, fs=25):
    """
    Creates a vespagram for the given slownessrange and slownessstepsize.
    Returns the vespagram as numpy array.
    ...
    :param power: Order of Nth-root stack, if None, just a linear stack is
                  performed.
    :type power: float

    :param markphases: Which phases should be marked,
                       default is 'ttall' + precursors, to mark all possible.
    ...
    vespagram = vespagram(stream, inv, cat[0], 3., 12., 0.1, power=4.)

    Author: S. Schneider, 2016
    Reference: Rost, S. & Thomas, C. (2002).
               Array seismology: Methods and Applications
    """

    # Prepare and convert objects.
    # Find geometrical center station of array.
    # If fails, the first trace is used.
    st = stream.copy()
    ...
    if method in ("fft"):
        # Calculate timeshift-table as a tensor,
        # see shift2ref method "fft" as guide.
        timeshift_table = np.zeros((data.shape[0], urange.size,
                                    dft.shape[1])).astype('complex')
        ...
        for i, u in enumerate(urange):
            for j, trace in enumerate(data):
                ...
                if epidist[j] > epidist[sref]:
                    _sdt, _si = shift2ref(trace, 0, sshift, method="normal")
                    shift_data_tmp[j, :] = _sdt
    ...
    # Plotting routine
    # plot_vespa(data=(vespa, taxis, urange), st=st, inv=inv, event=event,
    #            markphases=markphases, plot=plot, cmap=cmap, tw=tw,
    #            savefig=savefig, dpi=dpi, fs=fs, power=power, zoom=zoom)
